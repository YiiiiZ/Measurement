...
import yaml
# Initialization, Signal Map, Calibration, local oscillator freq finder

# def initialization(emulation):
...
def initialization(emulation: bool):
...
def calibration(qb_freq, qb_lo_freq, res_freq, res_lo_freq, drive_range, measure_range, acquire_range):
    ...
    exp_calibration["drive"] = SignalCalibration(
        oscillator = Oscillator(uid = "ch0_osc_0", frequency = qb_freq -qb_lo_freq, modulation_type=ModulationType.HARDWARE),
        local_oscillator = Oscillator(uid="ch0_lo", frequency = qb_lo_freq),
        range = drive_range,
    )
    exp_calibration["measure"] = SignalCalibration(
        oscillator = Oscillator(uid = "qa_osc_0", frequency = res_freq-res_lo_freq, modulation_type=ModulationType.HARDWARE),
        local_oscillator = Oscillator(uid="qa_lo", frequency = res_lo_freq),
        range = measure_range
    )
    exp_calibration["acquire"] = SignalCalibration(
        oscillator = Oscillator(uid = "qa_osc_1", frequency = res_freq-res_lo_freq, modulation_type=ModulationType.HARDWARE),
        local_oscillator = Oscillator(uid="qa_lo_1", frequency = res_lo_freq),
        range = acquire_range
    )
    return exp_calibration


def find_lo_freq(freq):
    step = 0.2e9  # 0.2 GHz = 2e8 Hz
